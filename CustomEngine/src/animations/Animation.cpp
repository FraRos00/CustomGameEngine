...
Animation::Animation(std::string ssPath, int startX, int startY, int frameWidth, int frameHeight, float scale,
              int frameCount, float frameSpeed, bool loop):ssPath(ssPath),startX(startX),startY(startY),frameWidth(frameWidth),
              frameHeight(frameHeight),scale(scale),frameCount(frameCount),frameSpeed(frameSpeed),loop(loop){
...
void Animation::Draw(Vector2 position, Color tint) const{
    ...
    if(spritesheet){
        ...
        Rectangle src{(float)frameX, (float)startY, (float)frameWidth, (float)frameHeight};
        DrawTexturePro(*spritesheet, src, dst, {frameWidth * scale / 2, frameHeight * scale / 2}, 0.0f, tint );
    }else{
